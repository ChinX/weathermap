...
func getCurrentWeather(city string) cache.Updater {
	...
	err := openweather.CurrentWeatherSummary(city,
		func(weather *openweather.CurrentWeather) error {
	...
	if err == nil {
		err = openweather.CurrentWeatherUltravioletIndex(su.CoordinatesLat, su.CoordinatesLon,
			func(weather *openweather.UltravioletIndex) error {
				su.UviDate = weather.Date
				su.UviDateISO = weather.DateISO
				su.UviValue = weather.Value
				return nil
			})
	}
